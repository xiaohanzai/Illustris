...
def inhullminmax(x, y, hull, minl, maxl, xmin, xmax, ymin, ymax):
    '''
    Return true if the point (x,y) is inside the convex hull
    and false otherwise
    '''
    R = math.sqrt(x * x + y * y)
    if R <= minl:
...
def gh_series(v, meanv, sigma, h3, h4):
    '''
    Return the value of a Gauss-Hermite series, used in estimating mean
    v, sigma, h3 and h4
    '''
...
def gh_gaussian(v, meanv, sigma):
    '''
    Return the value of a Gaussian, used in estimating mean v, sigma
    '''
    losvd = np.zeros(len(v))
...
def run(ifu_path, img_path):
    # read hull_file
    with open('%s/IFU_hull'%ifu_path, 'r') as f:
    ...
    hull_points = np.loadtxt('%s/IFU_hull'%ifu_path, skiprows=1)
    hullx = hull_points[:, 0]
    hully = hull_points[:, 1]

    # read ifu bin file
    bin_data = np.loadtxt('%s/voronoi_bins.dat'%ifu_path)
    ...
    data = np.load('%s/coordinates_star.npy'%img_path)
    ...
    IFU_data = open('%s/IFU_data'%ifu_path, 'w')

    for bin_index in range(num_ifu_bins):
